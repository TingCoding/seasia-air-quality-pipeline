...
[![CI](https://github.com/TingCoding/seasia-air-quality-pipeline/actions/workflows/ci.yml/badge.svg)](https://github.com/TingCoding/seasia-air-quality-pipeline/actions/workflows/ci.yml)
...
sql/fixtures/     a small deterministic dataset used by CI
...
- Explicit unit conversion so gaseous pollutants can be ingested

## Continuous integration

Every push runs the full stack against a real PostgreSQL instance: lint, the pytest
suite, the schema, a fixture dataset, and `dbt build` including all data quality tests.

The fixture is deliberately awkward. It contains a modelled hour with a NULL value, a
station hour that is absent rather than NULL, and two sensors at one station reporting
the same parameter — the three shapes that caused real problems during development.
Building against an empty database would only prove the SQL parses; feeding known rows
through proves it produces the right answer.
